using System;
using System.Collections.Generic;

namespace BusinessEntity.Data.Models;

public partial class ArcusfilSql
{
    public string CusNo { get; set; } = null!;

    public string? CusName { get; set; }

    public string SearchName { get; set; } = null!;

    public string? Addr1 { get; set; }

    public string? Addr2 { get; set; }

    public string? City { get; set; }

    public string? State { get; set; }

    public string Zip { get; set; } = null!;

    public string? Country { get; set; }

    public string? Contact { get; set; }

    public string? Contact2 { get; set; }

    public string PhoneNo { get; set; } = null!;

    public string? PhoneNo2 { get; set; }

    public string? PhoneExt { get; set; }

    public string? PhoneExt2 { get; set; }

    public string? FaxNo { get; set; }

    public int? StartDt { get; set; }

    public string SlspsnNo { get; set; } = null!;

    public string CusTypeCd { get; set; } = null!;

    public string? BalMeth { get; set; }

    public string? StmFreq { get; set; }

    public decimal? CrLmt { get; set; }

    public string? CrRating { get; set; }

    public string? HoldFg { get; set; }

    public string Collector { get; set; } = null!;

    public string? FinChgFg { get; set; }

    public string? CusOrigin { get; set; }

    public string? Filler0003 { get; set; }

    public string? Terr { get; set; }

    public string? CurrCd { get; set; }

    public string? ParCusNo { get; set; }

    public string? ParCusFg { get; set; }

    public string? ShipViaCd { get; set; }

    public string? UpsZone { get; set; }

    public string? ArTermsCd { get; set; }

    public decimal? DscPct { get; set; }

    public decimal? YtdDscGiven { get; set; }

    public string? TxblFg { get; set; }

    public string? TaxCd { get; set; }

    public string? TaxCd2 { get; set; }

    public string? TaxCd3 { get; set; }

    public string? ExemptNo { get; set; }

    public decimal? SlsPtd { get; set; }

    public decimal? SlsYtd { get; set; }

    public decimal? SlsLastYr { get; set; }

    public decimal? CostPtd { get; set; }

    public decimal? CostYtd { get; set; }

    public decimal? CostLastYr { get; set; }

    public decimal? Balance { get; set; }

    public decimal? HighBalance { get; set; }

    public int? LastSaleDt { get; set; }

    public decimal? LastSaleAmt { get; set; }

    public int? InvYtd { get; set; }

    public int? InvLastYr { get; set; }

    public int? PaidInvYtd { get; set; }

    public int? LastPayDt { get; set; }

    public decimal? LastPayAmt { get; set; }

    public int? AvgPayYtd { get; set; }

    public int? AvgPayLastYr { get; set; }

    public int? LastStmAgeDt { get; set; }

    public decimal? AmtAgePrd1 { get; set; }

    public decimal? AmtAgePrd2 { get; set; }

    public decimal? AmtAgePrd3 { get; set; }

    public decimal? AmtAgePrd4 { get; set; }

    public string? AllowSbItem { get; set; }

    public string? AllowBo { get; set; }

    public string? AllowPartShip { get; set; }

    public string? PrintDunnFg { get; set; }

    public string? Cmt1 { get; set; }

    public string? Cmt2 { get; set; }

    public string? VendNo { get; set; }

    public string? TaxSched { get; set; }

    public string? CrCard1Desc { get; set; }

    public string? CrCard1Acct { get; set; }

    public short? CrCard1ExpDt { get; set; }

    public string? CrCard2Desc { get; set; }

    public string? CrCard2Acct { get; set; }

    public short? CrCard2ExpDt { get; set; }

    public string? UserDefFld1 { get; set; }

    public string? UserDefFld2 { get; set; }

    public string? UserDefFld3 { get; set; }

    public string? UserDefFld4 { get; set; }

    public string? UserDefFld5 { get; set; }

    public short? DfltInvForm { get; set; }

    public string? Loc { get; set; }

    public string? Note1 { get; set; }

    public string? Note2 { get; set; }

    public string? Note3 { get; set; }

    public string? Note4 { get; set; }

    public string? Note5 { get; set; }

    public int? UserDt { get; set; }

    public decimal? UserAmount { get; set; }

    public decimal? AmtAgeOeTerm { get; set; }

    public string? CusAltAdrCd { get; set; }

    public string? RfcNo { get; set; }

    public string? EmailAddr { get; set; }

    public string? Filler0002 { get; set; }

    public decimal A4glidentity { get; set; }

    public string? NullFg { get; set; }

    public string? CountryCd { get; set; }

    public int? UbigeoCd { get; set; }

    public string? EiEmailAddr { get; set; }

    public string? ActiveFg { get; set; }
}
