...
namespace BusinessData.Data
{
    public class SygenopcRepository : ISygenopcRepository
    {
        ...
        public async Task<IEnumerable<IDictionary<string, object>>> F_ListarAccesosUsuarioSistema(SygenacsDTO parametros, ConnectionManager objConexion)
        {
        ...
        public async Task<IEnumerable<IDictionary<string, object>>> F_ListarAccesos(ConnectionManager objConexion)
        {
            this._context = new DbAcceso(objConexion.F_ObtenerCredencialesConfig());
            using var connection = _context.Database.GetDbConnection();
            // Definir la consulta SQL con parámetros
            string sql = "EXEC usp_SY_LIS_SYGENOPC_SQL";
            if (connection.State == ConnectionState.Closed)
                await connection.OpenAsync();
            // Ejecutamos la consulta con Dapper y mapeamos a una lista de diccionarios
